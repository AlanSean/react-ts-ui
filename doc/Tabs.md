...
fixedTop?: number;//开启吸顶并设置 top的值  单位px
isAnimate?:boolean;//容器是否开启动画
activeClass?:string;//tab选中的样式class
defaultBarClass?:string;//修改tab 条的样式class
tablistWidth?:string;//tablist的宽度 单位 px
defaultClass?:string;//tab默认样式class
...
pullOnLoad?: boolean//是否开启上拉加载  开启上拉加载 子组件内部有pullOnLoad函数 则会进行调用
